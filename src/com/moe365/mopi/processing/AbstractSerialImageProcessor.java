 ...
 */
public abstract class AbstractSerialImageProcessor<R> implements ImageProcessor<R>, BiFunction<VideoFrame, VideoFrame, R> {
	/**
	 ...
	 */
	protected Consumer<ImageProcessorResult<R>> resultConsumer;
	protected AbstractSerialImageProcessor(int frameMinX, int frameMinY, int frameMaxX, int frameMaxY, Consumer<ImageProcessorResult<R>> output) {
	...
	@Override
	pubilc void setHandler(BiConsumer<ImageProcessorResult<R>> handler) throws UnsupportedOperationException, IllegalStateException {
		synchronized (this) {
	...
	public void run() {
		try {
			while (!Thread.interrupted()) {
				...
				try {
					VideoFrame frameOnRef = frameOn.get();
					VideoFrame frameOffRef = frameOff.get();
					
					//check again, just to be safe
					if (frameOffRef != null && frameOnRef != null) {
						R result;
						try {
							result = apply(frameOnRef, frameOffRef);
						...
						VideoFrameMetadata frameOnMetadata = new VideoFrameMetadata(frameOnRef);
						VideoFrameMetadata frameOffMetadata = new VideoFrameMetadata(frameOffRef);
						if (this.resultConsumer != null)
							this.resultConsumer.accept(new ImageProcessorResult<>(frameOnMetadata, frameOffMetadata, result));
